<template>
  <div class="min-h-screen flex flex-col bg-[var(--bgColor-default)] text-[var(--fgColor-default)] transition-colors duration-300">
    <!-- Navbar -->
    <header class="sticky top-0 z-50 border-b border-zinc-800 bg-zinc-900 text-zinc-100 shadow-lg backdrop-blur-md bg-opacity-95">
      <div class="mx-auto flex max-w-5xl flex-wrap items-center gap-3 px-4 py-3 sm:gap-4 sm:px-6">
        <!-- Logo -->
        <Link href="/" class="flex shrink-0 items-center gap-2 font-semibold tracking-tight text-white hover:text-emerald-400 transition-all duration-200 transform hover:scale-105">
          <svg class="h-6 w-6 text-emerald-400 filter drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z" />
          </svg>
          <span class="text-xl font-bold bg-gradient-to-r from-white via-zinc-200 to-emerald-400 bg-clip-text text-transparent">Blog</span>
        </Link>
        ...
        <div class="ml-auto flex items-center gap-2 sm:gap-3">
          ...
          >
            <svg v-if="isDark" class="h-5 w-5 text-yellow-400 animate-spin-slow" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
            </svg>
            <svg v-else class="h-5 w-5 hover:text-indigo-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <!-- Flash notifications -->
    <div v-if="showNotice && flash.notice" class="border-b border-emerald-200 bg-emerald-50 px-4 py-2.5 text-center text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/80 dark:text-emerald-200 flex items-center justify-center gap-2 animate-fade-in">
      <svg class="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <span class="font-medium">{{ flash.notice }}</span>
      <button @click="showNotice = false" class="ml-2 hover:opacity-80 p-0.5 rounded-full hover:bg-emerald-200 dark:hover:bg-emerald-900 transition-colors">
    ...
    <div v-if="showAlert && flash.alert" class="border-b border-red-200 bg-red-50 px-4 py-2.5 text-center text-sm text-red-800 dark:border-red-900 dark:bg-red-950/80 dark:text-red-200 flex items-center justify-center gap-2 animate-fade-in">
      <svg class="h-4 w-4 text-red-600 dark:text-red-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </svg>
      <span class="font-medium">{{ flash.alert }}</span>
      <button @click="showAlert = false" class="ml-2 hover:opacity-80 p-0.5 rounded-full hover:bg-red-200 dark:hover:bg-red-900 transition-colors">
        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
      </button>
